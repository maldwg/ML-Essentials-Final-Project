...
def search():
    ...
    for json_dump in create_dicts():
        ...
        p_main = subprocess.Popen(
            "python main.py play --agents baby_terminator --n-rounds=150 --train 1 --scenario coin-heaven --no-gui",
            shell=True,
        )
        ...
        if exit_code == 0:
            ...
            if len(top_ten_models) == 10:
                worst_model_id, worst_model_score = min(
                    top_ten_models, key=lambda x: x[1]
                )
                if mean_reward > worst_model_score:
                    print(
                        f"model {grid_search_model_id} with score {mean_reward} is better than worst model {worst_model_id} with score {worst_model_score}, replacing... "
                    )
