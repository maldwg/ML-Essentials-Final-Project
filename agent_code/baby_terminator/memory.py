...
class ReplayMemory:
    ...
    def recalculate_rewards(self, events):
        if c.MOVED_TOWARDS_COIN in events:
            self.game_rewards[c.MOVED_TOWARDS_COIN] = (
                1
                / (len(self.shortest_paths_to_coin[0]) - 1)
                * self.game_rewards_original[c.MOVED_TOWARDS_COIN]
            )
        if c.MOVED_TOWARDS_END_OF_EXPLOSION in events:
            self.game_rewards[c.MOVED_TOWARDS_END_OF_EXPLOSION] = (
                1
                / (len(self.shortest_paths_out_of_explosion[0]) - 1)
                * self.game_rewards_original[c.MOVED_TOWARDS_END_OF_EXPLOSION]
            )
